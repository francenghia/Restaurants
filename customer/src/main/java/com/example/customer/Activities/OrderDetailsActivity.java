...
import com.example.customer.Adapters.OrderDetailsRecyclerAdapter;
...
import static com.example.common.Shared.STATUS_UNKNOWN;
...
public class OrderDetailsActivity extends AppCompatActivity {
    ...
    private void insertItems (){
        ...
        //Set total
        double amount = Double.parseDouble(item.getTotPrice());
        DecimalFormat formatter = new DecimalFormat("#,###");
        ((TextView)findViewById(R.id.order_det_tot1)).setText(formatter.format(amount)+" vnd");
        ((TextView)findViewById(R.id.order_det_tot)).setText(formatter.format(amount)+" vnd");
        //Set customer addr
...
//        });
        ...
        for(Map.Entry<String, Integer> entry : knv.entrySet()) {
            keys.add(entry.getKey());
            nums.add(entry.getValue().toString());
        }
